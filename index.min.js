...
// Initialize scriptLoadDateNormalized at the very beginning
// This captures the date when the script was initially loaded, normalized to midnight.
var scriptLoadTime = new Date();
var scriptLoadDateNormalized = new Date(scriptLoadTime.getFullYear(), scriptLoadTime.getMonth(), scriptLoadTime.getDate());
scriptLoadDateNormalized.setHours(0, 0, 0, 0); // Normalize to the beginning of the day the script loaded
...
function render() {
    ...
    var daysPassed = Math.floor((currentDateNormalized.getTime() - scriptLoadDateNormalized.getTime()) / millisecondsPerDay);

    var dailyAccumulatedOffsetms = 0;
    // Use config.ofs (parsed float, defaults to 0)
    if (!isNaN(config.ofs) && config.ofs !== 0) { // config is now defined before render() is called
        dailyAccumulatedOffsetms = daysPassed * config.ofs;
    }

    // Apply the accumulated offset to the current time
    var time = new Date(actualCurrentTime.getTime() + dailyAccumulatedOffsetms); // This 'time' is the adjusted local time base
    var len = time.getTime();
    var offset = time.getTimezoneOffset() * 60000;
    var utcTime = len + offset;
    var date = new Date(utcTime + 3600000 * config.timezone); // config.timezone is used here
    ...
    var timeText = "".concat(date.getHours(), ":").concat(String(date.getMinutes()).padStart(2, '0')); // Ensure minutes are two digits
...
var config = {
    ...
    rotate: urlQuery.r,
    lang: urlQuery.l, // FIXED: Added comma here
...
// Ensure config.rotate is handled correctly if undefined
domApp.style.transform = `rotate(${config.rotate || 0}deg) translate3d(-50%,-50%,0)`;
// Note: -webkit-transform is an older prefix. `transform` is more standard.
// If you need -webkit-transform specifically:
// domApp.style.cssText = `-webkit-transform: rotate(${config.rotate || 0}deg) translate3d(-50%,-50%,0); transform: rotate(${config.rotate || 0}deg) translate3d(-50%,-50%,0);`;


// Initial render call and interval setup for periodic updates
// Moved after config initialization
render();
